...
# モデルの構築
# 目的変数がone-hotエンコードされている場合、lossを「categorical_crossentropy」とする
model.compile(
  optimizer = "rmsprop",
...
```{python}
#| label: predictで予測を行なう
Y_pred_0 = model.predict(X_test)
```

各行で一番数値が大きい＝確率が高い列の番号を抽出する。
argmaxはリストで一番大きい値のインデックスを返す関数（「axis=1」は列方向を表す）

```{python}
# 多クラス分類は予測結果の各リストにおける最大値のインデックスを取得するようにする
Y_pred = np.argmax(Y_pred_0, axis = 1)
Y_pred
```

比較のため、カテゴリー変数に展開した Y_test の目的変数をクラス番号に復元
